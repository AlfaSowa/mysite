...
export default class Works extends React.Component {
	...
	render() {
		...
		return (
			<ul className='works__list'>{works !== [] ? works.map((item, index) => <Block setRef={this.state.getWorksRef} key={index} index={index} item={item} showInfo={this.showInfo} />) : null}</ul>
		);
	}
}
